...
read_facereader <- function(file, subject = NULL, skip = NULL, ...){
  ...
  source <- as.character(src[position,1])
  ...
  if(include=="Basic"){
    ...
    for (i in seq_along(emotions)) {
      if(emotions[[i]] %in% vars){
        data[,emotions[i]] <- as.numeric(ifelse(data[,emotions[j]] == 'FIT_FAILED',NA,data[,emotions[i]]))
      } else next
    }

  } else if(include =='All') {

    nums <- c('Neutral','Happy','Sad','Angry','Surprised','Scared',
                 'Disgusted','Valence','Arousal', 'Contempt',
                 'Y...Head.Orientation','X...Head.Orientation',
                 'Z...Head.Orientation')
    facts <- c('Gender','Age','Beard','Moustache','Ethnicity',
               'Mouth','Left.Eye','Right.Eye','Left.Eyebrow',
               'Right.Eyebrow','Glasses','Landmarks','Stimulus','Event.Marker')

    for (i in seq_along(nums)) {
      if(nums[[i]] %in% vars){
        data[,nums[i]] <- as.numeric(ifelse(data[,nums[i]] == 'FIT_FAILED',NA,data[,nums[i]]))
      } else next
    }
    ...
    for (j in seq_along(facts)) {
      if(facts[[j]] %in% vars){
        ...
        data[,facts[j]] <- ifelse(data[,facts[j]] == 'FIT_FAILED',NA, data[,facts[j]])
    ...
    for (k in seq_along(facts)) {
      if(facts[[k]] %in% vars){
        ...
        data[,facts[k]] <- ifelse(data[,facts[k]] == 'FIND_FAILED',NA, data[,facts[k]])
...
score_aus <- function(data){
  ...
  data[AUs] <- purrr::map_df(data[AUs], ~dplyr::recode(.x, A=1,B=2,C=3,D=4,E=5,
                                                       NotActive=0))
